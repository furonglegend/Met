...
## 🔧 Phase 3: 原生 LoRA 内嵌编辑 (PEFT 直接优化)
...
- 冻结所有被选层原始权重 (requires_grad=False)。
- 注入 LoRA 对 (A,B)；编辑求解输出 ΔW，经 SVD 截断映射到低秩：ΔW ≈ B @ A。
- 支持两种求解路径：
  1) 闭式：SVD 截断前 r 分量直接赋值。
  2) 拟合：微步梯度 (≤10 steps) 最小化 ‖W_base + scale*(B@A) − (W_base + ΔW_target)‖_F。
- 每次编辑分配 fact_id，记录其对应的 (A,B) 当前增量；信任评分不达阈值 → 清零该增量（局部回滚）。
- Replay 采样仅影响新 LoRA 参数的拟合或 SVD 输入，不修改主权重。
- 统计：参数增量占比 / 残差范数 / rank vs 成功率。
...
- Param Efficiency (新增参数 / 总参数) ≤ 2% (≥500 facts)
- 历史编辑保留率相对 raw 提升 ≥ 10%
- 平均 NS 恶化降低 ≥ 20%
- 残差均值 ‖ΔW − B@A‖_F / ‖ΔW‖_F ≤ 0.15
- Fallback 率 ≤ 5%
...
| 风险 | 缓解 |
|------|------|
| SVD 数值不稳定(大矩阵) | 层分块 / 使用经济型 SVD |
| Rank 过低导致编辑失败 | 动态提示提升 rank 重试 |
| 多事实共享 LoRA 导致串扰 | 按事实分离增量 (fact_id 缓冲) |
| 回滚后残留影响 | 回滚后立即重新评测校验 |
| 显存上涨 | 延迟注入：仅在被编辑层插入 LoRA |
